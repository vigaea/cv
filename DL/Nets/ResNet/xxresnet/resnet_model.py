...
class ResNet(object):
    ...
    def _build_model(self):
        with tf.variable_scope('init'):  # scope类似于c++的作用域，用于参数共享
            x = self._images
            """第一层卷积（3,3x3/1,16）"""
    ...
    def _bottleneck_residual(self, x, in_filter, out_filter, stride,
        ...
        # 第1子层
        with tf.variable_scope('sub1'):
            # 1x1卷积，使用输入步长，通道数(in_filter -> out_filter/4)，降四倍
    ...
    # x通道特征的输入，mean均值，var方差，得到0-1的正态分布，gama尺度，beta偏移
    def _batch_norm(self, name, x):
        with tf.variable_scope(name):
            # 输入通道维数
            params_shape = [x.get_shape()[-1]]
            # 作用域_beta，判断变量是否存在，有则提取使用，否则新建
            # offset
